...
if __name__ == "__main__":
    ...
    for name, module in model_backbone.named_modules():
        if isinstance(module, torch.nn.Linear):
            ...
            prune.l1_unstructured(module, name='bias', amount=0.8)
            prune.remove(module, 'weight')
            prune.remove(module, 'bias')
        model_params_lo_pruning = model_params_lo_pruning + parameter.numel()
    print('INFO: L1_norm local pruning parameter count:', model_params_lo_pruning)
    
    
    # Global 프루닝 후 파라미터 출력
    parameters_to_prune = ()
    
    for i in range(5):
        parameters_to_prune += (
            (model_backbone.blocks_st[i], 'weight')
        )
        model_params_glo_pruning = model_params_glo_pruning + parameter.numel()
    prune.global_unstructured(
        parameters_to_prune,
        pruning_method = prune.L1Unstructured,
        amount = 0.9
    )
    print('INFO: L1_norm global pruning parameter count:', model_params_glo_pruning)
