...
def train_with_config(args, opts):
    ...
    print('INFO: Baseline model trainable parameter count:', model_params)
    
    #KDH dynamice quantization 
    model_backbone = torch.quantization.quantize_dynamic(
        model_backbone, {torch.nn.Linear}, dtype=torch.qint8
    )
    #
    ...
    model_params2 = 0
    for parameter in model_backbone.parameters():
        model_params2 = model_params2 + parameter.numel()
    print('INFO: Quantized model trainable parameter count:', model_params2)
    """
    
    # KDH quantization parameter count
    #qt_model_params = 0
    #for parameter in quantized_model.parameters():
    #    qt_model_params = qt_model_params + parameter.numel()
    #print('INFO: Quantized model trainable parameter count:', model_params)
    #
...
if __name__ == "__main__":
    ...
    #KDH static quantization 
    backend = "qnnpack"
    model_backbone.qconfig = torch.quantization.get_default_qconfig(backend)
    torch.backends.quantized.engine = backend
    model_static_quantized = torch.quantization.prepare(model_backbone, inplace=False)
    ...
    """
    model_backbone = load_backbone(args)
    print(count_parameters(model_backbone))
