...
def parse_args():
    ...
    #KDH parser 추가
    parser.add_argument('--pruning', default='', type=float, help='pruning amount', required=True)
    #
...
def evaluate(args, model_pos, test_loader, datareader):
    ...
    for action in action_names:
        final_result.append(round(np.mean(results[action]),4))
        final_result_procrustes.append(round(np.mean(results_procrustes[action]),4))
    summary_table.add_row(['P1'] + final_result)
    summary_table.add_row(['P2'] + final_result_procrustes)
    print(summary_table)
    e1 = round(np.mean(np.array(final_result)),4)
    e2 = round(np.mean(np.array(final_result_procrustes)),4)
...
def train_with_config(args, opts):
    ...
    """
    # KDH Global unstructured(i.e., weight) 프루닝 실행
    ...
    for i in range (5): 
        ...
        # block_st pruning
            #attn_s    
            (model_backbone.blocks_st[i].attn_s.proj, 'weight'),
            (model_backbone.blocks_st[i].attn_s.qkv, 'weight'),
            #attn_t
            (model_backbone.blocks_st[i].attn_t.proj, 'weight'),
            (model_backbone.blocks_st[i].attn_t.qkv, 'weight'),
            #mlp_s
            (model_backbone.blocks_st[i].mlp_s.fc1, 'weight'),
            (model_backbone.blocks_st[i].mlp_s.fc2, 'weight'),
            #mlp_t
            (model_backbone.blocks_st[i].mlp_t.fc1, 'weight'),
            (model_backbone.blocks_st[i].mlp_t.fc2, 'weight'),
    ...
    prune.ln_structured(model_backbone, name='weight', amount=0.8, dim=1)
    prune.ln_structured(model_backbone, name='weight', amount=0.8, dim=1)
    prune.ln_structured()
...
if __name__ == "__main__":
    ...
    for parameter in model_backbone.parameters():
        model_params = model_params + parameter.numel()
    print('INFO: Original parameter count:', model_params)
    
    #train_with_config.prune.global_unstructured
    
    # sparisity 계산
    numerator, denominator = 0, 0
    for i in range(5):
        numerator += torch.sum(model_backbone.blocks_st[i].attn_s.proj.weight == 0)
        numerator += torch.sum(model_backbone.blocks_st[i].attn_s.qkv.weight == 0)
        numerator += torch.sum(model_backbone.blocks_st[i].attn_t.proj.weight == 0)
        numerator += torch.sum(model_backbone.blocks_st[i].attn_t.qkv.weight == 0)
        numerator += torch.sum(model_backbone.blocks_st[i].mlp_s.fc1.weight == 0)
        numerator += torch.sum(model_backbone.blocks_st[i].mlp_s.fc2.weight == 0)
        numerator += torch.sum(model_backbone.blocks_st[i].mlp_t.fc1.weight == 0)
        numerator += torch.sum(model_backbone.blocks_st[i].mlp_t.fc2.weight == 0)
        
        denominator += model_backbone.blocks_st[i].attn_s.proj.weight.nelement()
        denominator += model_backbone.blocks_st[i].attn_s.qkv.weight.nelement()
        denominator += model_backbone.blocks_st[i].attn_t.proj.weight.nelement()
        denominator += model_backbone.blocks_st[i].attn_t.qkv.weight.nelement()
        denominator += model_backbone.blocks_st[i].mlp_s.fc1.weight.nelement()
        denominator += model_backbone.blocks_st[i].mlp_s.fc2.weight.nelement()
        denominator += model_backbone.blocks_st[i].mlp_t.fc1.weight.nelement()
        denominator += model_backbone.blocks_st[i].mlp_t.fc2.weight.nelement()

    # Pruning 파라미터 계산 
    model_params_glo_pruning = 0   
    pruning_amount = args.pruning 
    for parameter in model_backbone.parameters():
        model_params_glo_pruning = (model_params_glo_pruning + parameter.numel())*(pruning_amount)
    print('INFO: Pruned parameter count:', int(model_params_glo_pruning))
    print("Global sparsity: {:.4f}%".format(100. * float(numerator) / float(denominator)))